...
public class practice13 {
    public static void main(String[] args) {
        ...
        // 문제 2]
        Flyable flyable = new Duck(); // Duck 클래스가 flyable 구현했기 때문에 가능하다.( 다형성 : 타입변환 가능 )
        Swimmable swimmable = new Duck(); // Duck 클래스가 swimmable 구현했기 때문에 가능하다.( 다형성 : 타입변환 가능 )
        flyable.fly();
        swimmable.swimmable();
...
class Cat implements Soundable{ // 해당 추상메소드를 가진 인터페이스를 구현하면 그것은 구현(객)체라고 한다.
