...
-- insert into test( mno , mcount ) values( 5 , 25 ); -- 오류발생 Field 'mname' doesn't have a default value
# 6) unique 제약조건 속성은 중복값이 불가능하다.
-- insert into test( mname ) values( "유재석" ); -- 오류발생 Duplicate entry '유재석' for key 'test.mname'	
# 7) 모든 속성에 대해 값 삽입할 경우에 속성명 생략 가능하다. 값 대응시 속성명 순서 일치
insert into test values( 5 , "박명수" , 60 );
# 8) 하나의 명령어에서 다수 레코드를 삽입 , values( 레코드1 ) , ( 레코드2 ) , ( 레코드3 );
insert into test values( 6 , "하하" , 70 ) , ( 7 , "박진감" , 1 ) , ( 8 , "박소영" , 10 );
# 확인
select * from test;

-- [2] SELECT , 레코드 조회 , 주의할 점 : 조회결과 테이블과 원본테이블은 상관이 없다.
# 1) select * from 테이블명;
select * from test;
# 2) select 속성명1 , 속성명2 , 속성명3 from 테이블명; : 조회하고 싶은 속성만 골라서 조회 가능
select mname from test;
select mname , mcount from test;
# 3) select 속성명 from 테이블명 where 조건절
select * from test where mname = "박진감"; -- SQL에서는 = 같다 기호로 사용된다. vs 자바에서는 = 대입
select * from test where mcount >= 40; -- 조건절에서는 연산 이용한 조회가 가능하다.

-- [3] UPDATE , 레코드의 값 수정 , mysql workbench safeMode 해제 :
...
select * from test; -- 전체의 mcount 가 0으로 변함
# 2) 일반적으로 전체수정보다는 조건 수정을 한다.
update test set mcount = 10 where mname = "유재석"; -- mname 속성값이 '유재석'이면 mcount 속성값을 10으로 변경한다.
update test set mcount = 20 , mname = "강호동2" where mno = 2; -- mno 속성값이 2 이면 mcount 속성값을 20 , mname 속성값을 '강호동2'로 변경해라.

-- [4] DELETE , 레코드 삭제 , ~차이점~ delete : 레코드( 한 줄 ) 삭제 VS truncate : 모든 레코드 삭제 VS drop : 테이블/데이터베이스 삭제
# 1) delete from 테이블명 where 조건
delete from test where mno = 2; -- mno 속성값이 2이면(강호동레코드) 삭제해라.
# 2) delete from 테이블명 : 전체 삭제
delete from test; -- 조건이 없으므로 모든 레코드 삭제 			- DML 취소가능
truncate table test; -- 해당 테이블의 모든 내용(레코드) 삭제		- DDL 취소불가능
...
# 1] 별칭 키워드 , 조회결과의 속성명 변경 , as 생략가능하다. 속성명/테이블명이 길거나 복잡한 경우에 사용
...
# 3] 산술연산자 , 주의할 점 : select 결과는 !!!!!!!!!!!!!!!!!!!항상 표/테이블!!!!!!!!!
...
select * from member where mname like "에이_";								-- manme 속성값이 '에이'로 시작하면서 3글자 문자
select * from member where mname like '%핑크';								-- mname 속성값이 '핑크'로 끝나는 문자
select * from member where mname like '%이%';								-- mname 속성값 중 '이'가 포함된 문자 
