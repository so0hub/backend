...
# 서로 다른 테이블 간의 교집합(내부조인) 찾기 : pk속성명과 fk속성명 , 카테고리번호_pk = 카테고리번호_fk
...
select product.제품명,제품가격 from pcategory inner join product on pcategory.카테고리번호_pk = product.카테고리번호_fk;

-- [문제 3] 모든 제품의 제품명과 등록된 재고수량을 함께 조회하세요. (조회결과: 8개 레코드)
select * from product p inner join stock s on p.제품번호_pk = s.제품번호_fk; # 이건 전체 다 나오는 거 제품번호까지
...
select stock.재고등록날짜,재고수량 from product inner join stock on product.제품번호_pk = stock.제품번호_fk where product.제품명 = '그램 15인치';

-- [문제 5] 모든 제품의 제품명, 카테고리명, 재고수량을 한 번에 조회하세요. (3개 테이블 조인) (조회결과: 8개 레코드)
-- select product.-- pcategory( 카테고리 번호 ) <--> product( 카테고리 번호 ) , product( 제품번호 ) <--> stock( 제품번호 )
...
select p.제품명 from product p left join stock s on p.제품번호_pk = s.제품번호_fk where s.제품번호_fk is null;
-- left join 쓰는 이유 : 재고가 없는 제품까지 일단 다 불러오기 위해.
-- = null이 아니라 is null이다!


-- [문제 8] 각 카테고리별로 총 재고 수량의 합계를 카테고리명과 함께 조회하세요. (조회결과: 3개 레코드)
select pc.카테고리명 , sum(s.재고수량) as 총재고수량 from pcategory pc 
