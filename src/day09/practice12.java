...
3. main 함수에서 Student 객체를 생성하고, 상속받은 name과 자신의 studentId에 값을 저장한 뒤 모두 출력하세요. */
        ...
        student.name = "유재석"; // 상속받은 멤버변수 name에 값
        student.studentId = 30; // 현재타입의 멤버변수 studentID
...
명하세요.*/
        ...
        Figure figure = triangle; // Figure 타입의 변수에 저장

        // Triangle 클래스는 Figure로부터 상속받았기 때문에 가능하다.
...
2. Person 클래스를 상속받는 Student 클래스를 만드세요. Student 클래스에는 studentId(정수) 멤버 변수를 추가하세요.
3. main 함수에서 Student 객체를 생성하고, 상속받은 name과 자신의 studentId에 값을 저장한 뒤 모두 출력하세요. */
        class Person { // 클래스 첫 글자 대문자로 한다.
            String name;
        }
        class Student extends Person{ // extends : 상속키워드(다른 클래스의 멤버들을 확장/연장)
            int studentId;
        }
...
3. main 함수에서 Cat 객체를 생성하고 makeSound() 메소드를 호출하여, 재정의된 내용이 출력되는지 확인하세요.*/
    ...
    // 오버라이딩 : 물려받은 메소드 재정의 , 오른쪽 클릭 생성 -> 자동완성
            @Override
            void makeSound() {
